import { useCallback } from "react";
import { useCalendars } from "../../contexts/CalendarsContext";
import { useSeasons } from "../../contexts/SeasonsContext";
import { useCalendarSeasons } from "../../contexts/CalendarSeasonsContext";
import { useDriverProfiles } from "../../contexts/DriverProfilesContext";
import { calculateAndSaveDriverStats } from "../utils/calculateDriverStats";

export function useCalculateDriverStats() {
	const { allCalendars } = useCalendars();
	const { seasons } = useSeasons();
	const { mappings } = useCalendarSeasons();
	const { profiles } = useDriverProfiles();

	const triggerForGrid = useCallback(
		async (gridId: string) => {
			const gridCalendarIds = new Set(
				allCalendars.filter((c) => c.grid === gridId).map((c) => c.id),
			);
			const gridSeasonIds = new Set(
				mappings
					.filter((m) => gridCalendarIds.has(m.calendarId))
					.map((m) => m.seasonId),
			);
			const activeSeason =
				seasons.find((s) => s.active && gridSeasonIds.has(s.id)) ??
				seasons.find((s) => s.active) ??
				seasons
					.filter((s) => gridSeasonIds.has(s.id))
					.slice(-1)[0];

			if (!activeSeason) return;

			const seasonCalendarIds = new Set(
				mappings
					.filter((m) => m.seasonId === activeSeason.id)
					.map((m) => m.calendarId),
			);

			// All season calendars for this grid, with results
			const seasonCalendars = allCalendars
				.filter(
					(c) =>
						c.grid === gridId &&
						seasonCalendarIds.has(c.id),
				)
				.map((c) => c.id);

			// All career calendars for this grid (all grid calendars)
			const careerCalendars = allCalendars
				.filter((c) => c.grid === gridId)
				.map((c) => c.id);

			// All driver IDs in this grid from profiles
			const driverIds = Object.entries(profiles)
				.filter(([, gridProfiles]) => !!gridProfiles[gridId])
				.map(([driverId]) => driverId);

			if (driverIds.length === 0) return;

			await calculateAndSaveDriverStats(
				gridId,
				seasonCalendars,
				careerCalendars,
				driverIds,
			);
		},
		[allCalendars, seasons, mappings, profiles],
	);

	return { triggerForGrid };
}

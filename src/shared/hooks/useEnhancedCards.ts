...
import useCsvLoader from "../../components/hooks/useCsvLoader";
import { useGetTeamsQuery } from "../../graphql/generated";
import { GridId } from "../../components/config/grids";
        ...
        return cards
            .map((card: any) => {
                const driverFromData = data?.drivers?.find((d: any) =>
                    card.id ? d.id === card.id : normalizeString(d.name) === normalizeString(card.name),
                );
                ...
                const driverStats = stats.find((stat: any) =>
                    card.id ? stat.id === card.id : normalizeString(stat.name) === normalizeString(card.name),
                );

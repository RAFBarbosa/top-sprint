...
export function useStandings(
    ...
    const gridDriversData = drivers.filter(
        activeTab === "drivers" ? (item) => item.grid === "gridA" : (item) => item.grid === "gridB",
    );
    const gridTeamsData = teams;

    const displayedData = showAll ? gridDriversData.slice(3) : gridDriversData.slice(3, 10);

    const displayedDataMobile = showAll ? gridDriversData : gridDriversData.slice(0, 10);
